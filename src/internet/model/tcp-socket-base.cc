...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddTraceSource ("ECNCWRSeq",
                     ...
                     "ns3::SequenceNumber32TracedValueCallback")
    .AddTraceSource ("RECNLastNonceSeq",
...
TcpSocketBase::TcpSocketBase (void)
  : TcpSocket (),
    ...
    m_ecnCWRSeq (0),
    m_recn (false),
    m_recn_nonce_sum_sender (1),
    m_recn_nonce_sum_receiver (1),
    m_recn_nonce_synch (0),
    m_recn_lastNonceSeq (0)
{
      ...
      if(m_ecn && m_recn && (tcpHeader.GetFlags () & (TcpHeader::CWR | TcpHeader::ECE | TcpHeader::NS)) == (TcpHeader::CWR | TcpHeader::ECE | TcpHeader::NS))
        {
          ...
          SendEmptyPacket (TcpHeader::SYN | TcpHeader::ACK | TcpHeader::ECE | TcpHeader::NS);
          m_tcb->m_ecnState = TcpSocketState::ECN_IDLE;
...
TcpSocketBase::SendEmptyPacket (uint16_t flags)
{
  ...
  if(m_tcb->m_recn_state && (flags & TcpHeader::ACK))
    {
      if(m_rxBuffer->NextRxSequence() != m_recn_lastNonceSeq)
        {
          while(!m_tcb->NonceOutOfOrder.empty() && m_tcb->NonceOutOfOrder.top().first < m_recn_lastNonceSeq  )
            {
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (GetIpTos ())
    ...
    {
      if (m_tcb->m_ecnState != TcpSocketState::ECN_DISABLED && !isRetransmission && m_tcb->m_recn_state)
        {
          SocketIpTosTag ipTosTag;
          uint8_t temp = 0x01;
          ipTosTag.SetTos (temp);
